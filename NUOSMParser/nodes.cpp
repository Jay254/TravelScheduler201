...
bool Nodes::find(long long id, double &lat, double &lon, bool &isEntrance) const
{
  ...
  // linear search:
  //
  // for (const Node& N : this->MapNodes)
  // {
  //   if (N.getID() == id) {
  //     lat = N.getLat();
  //     lon = N.getLon();
  ...
  //
  int low = 0;                               // low index
  int high = (int)this->MapNodes.size() - 1; // high index

  while (low <= high)
  {
    int mid = low + ((high - low) / 2); // middle index

    long long nodeid = this->MapNodes[mid].getID(); // get node id at middle index

    if (id == nodeid) // found
    {
      lat = this->MapNodes[mid].getLat();               // get latitude at middle index
      lon = this->MapNodes[mid].getLon();               // get longitude at middle index
      isEntrance = this->MapNodes[mid].getIsEntrance(); // get isEntrance at middle index

      return true;
    }
    else if (id < nodeid) // search left:
    {
      high = mid - 1; // // adjust the high index for left half search.
    }
    else // search right:
    {
      low = mid + 1; // adjust the low index for right half search.
    }
  } // end while
